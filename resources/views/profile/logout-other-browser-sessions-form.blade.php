<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Administre los navegadores y dispositivos en los que tiene una sesión activa.') }}
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div class="d-flex mt-3 justify-content-end">
            <button class="btn btn-outline-danger text-uppercase" wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('Cerrar sesiones activas en otros navegadores') }}
            </button>
        </div>

        <!-- Log out Other Devices Confirmation Modal -->
        <x-jet-dialog-modal wire:model="confirmingLogout" maxWidth="lg">
            <x-slot name="title">
                {{ __('Cerrar sesión en otros navegadores') }}
            </x-slot>

            <x-slot name="content">
                {{ __('Por favor ingrese su contraseña para confirmar que quiere cerrar las sesiones activas en otros navegadores y dispositivos.') }}
            ...
            <x-slot name="footer">
                ...
                <button class="btn btn-outline-danger text-uppercase" wire:click="logoutOtherBrowserSessions" wire:loading.attr="disabled">
                    <div wire:loading wire:target="logoutOtherBrowserSessions" role="status">
